...
int aes_crypt_ecb( aes_context* ctx, int mode, const unsigned char* input, int slen, unsigned char* output);


int ALG_AES_EnCrypt(const char* input, int slen, const char* key, int keysize, char* output)
{
    aes_context ctx;
    aes_setkey_enc(&ctx, (const unsigned char*)key, keysize);
    return aes_crypt_ecb(&ctx, AES_ENCRYPT, (const unsigned char*)input, slen, (unsigned char*)output);
...
{
    aes_context ctx;
    aes_setkey_dec(&ctx, (const unsigned char*)key, keysize);
    return aes_crypt_ecb(&ctx, AES_DECRYPT, (const unsigned char*)input, slen, (unsigned char*)output);
}
